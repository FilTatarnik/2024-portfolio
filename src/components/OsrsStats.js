...
const OsrsStats = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
      <div className="bg-gray-900 text-white p-6 rounded-lg shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {stats.length > 0 && (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4">
            {stats.map((skill, index) => (
              <div key={index} className="flex flex-col items-center bg-gray-800 p-3 rounded-md">
                <img
                  src={`/${skillNames[index].toLowerCase()}_icon.webp`}
                  alt={skillNames[index]}
