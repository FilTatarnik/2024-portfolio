...
const PokemonCards = ({ isOpen, onClose }) => {
  ...
  const [ownedCards, setOwnedCards] = useState([
    3, 9, 18, 26, 31, 34, 36, 38, 40, 45, 62, 65, 68, 71, 82, 89, 94, 97, 101,
    106, 107, 110, 112, 113, 115, 117, 122, 123, 127, 130, 131, 132, 135, 136,
      ...
      const pokemonList = await Promise.all(
        data.results.map(async (pokemon, index) => {
          ...
          const relevantStats = [
            {
              name: "hp",
              value: details.stats.find((s) => s.stat.name === "hp").base_stat,
            },
            {
              name: "attack",
              value: details.stats.find((s) => s.stat.name === "attack")
                .base_stat,
            },
            {
              name: "defense",
              value: details.stats.find((s) => s.stat.name === "defense")
                .base_stat,
            },
            {
              name: "speed",
              value: details.stats.find((s) => s.stat.name === "speed")
                .base_stat,
            },
          ];
  ...
  return (
    ...
    >
      <div
        className="bg-gray-800 rounded-lg shadow-lg max-w-4xl mx-auto w-full max-h-[90vh] overflow-y-auto"
            ...
            {allCards.map((card) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <h2 className="text-white text-center text-sm capitalize">
                    {card.name}
                  </h2>
                  {!isOwned && (
